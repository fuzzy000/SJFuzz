...
import com.djfuzz.differential.DifferentialFramework;
...
import com.djfuzz.solver.JVMOutputParser;
import com.djfuzz.solver.OutputParser;
...
public class DjfuzzFrameworkResumableDiffGuide {
    ...
    private static ThreadPool jvmThreadPool = DjfuzzFrameworkResumableAdvanced.getJvmThreadPool();
    private static OutputParser outputParser = new JVMOutputParser();

    public static DifferentialFramework getDiffFramework() {
        return diffFramework;
    }

    private static DifferentialFramework diffFramework = new DifferentialFramework();

    DjfuzzFrameworkResumableDiffGuide(){
    ...
    public static void setActionContainer(Map<String, Action> actionContainer) {
        DjfuzzFrameworkResumableDiffGuide.actionContainer = actionContainer;
    }
    ...
    public void process(String className, int iterationLimit, String[] args, String classPath, String dependencies, String jvmOptions) throws Exception {
        jvmThreadPool.start();
        ...
        EnvironmentSetting.generateJunitJvmAndCmd("");
        ...
        FlatMutateClass.switchSelectStrategy();
        Main.initial(args);
        ...
        if(mutateAcceptHistory.size() == 0){
            MutateClass mutateClass = new FlatMutateClass();
            mutateClass.initialize(className, args, null, jvmOptions);
//            fillClassWithOutput(mutateClass);
            ...
            DjfuzzFrameworkResumableDiffGuide.getDiffFramework().submitClass(startState);
            checkBackpathNull(mutateAcceptHistory, "In initialization, mutateAcceptHistory:(size="+mutateAcceptHistory.size()+")");
            DifferentialLogger.clearRecords();
            UniquenessJudge.clearRecords();
        }else{
            // moved to the method readCurrentPopulation
//            diffFramework.recovery(classPath, className, args, jvmOptions);
//            UniquenessJudge.recover();
        }
...
//                Action nextAction = DjfuzzFrameworkResumableDiffGuide.getActionContainer().get(nextActionString);
//                State nextState = nextAction.proceedAction(current.getTarget(), mutateAcceptHistory);
//                iterationCount ++;
//                MutateClass newOne = nextState.getTarget();
//
////                fillClassWithOutput(newOne);
//
//                if (newOne != null) {
...
//        System.out.println(MathTool.mean(totalScore));
        G.reset();
        jvmThreadPool.close();
        diffFramework.getPool().close();
    }

    public static void fillClassWithOutput(MutateClass mutateClass){
        try{
            FlatMutateClass flatMutateClass = (FlatMutateClass) mutateClass;
            List<Task> tasks = EnvironmentSetting.generateTaskAccordingToClass();
            for (Task task: tasks){
                jvmThreadPool.submit(task);
            }
            for (Task task: tasks){
                flatMutateClass.addJvmOutputResults((outputParser.parseOutput(jvmThreadPool.retrieve().getInputInfo())));
            }
        }catch (Throwable t){
            t.printStackTrace();
        }
    ...
    public static List<State> readCurrentPopulation(String populationPath, String classPath, String className, String[] args, String jvmOptions) {
        ...
        for(File f: file.listFiles()){
            ...
            // because during loading it will use UniquenessJudge.
            if (loadingFirst){
                try{
                    // must be first because diffFramework.recovery will use UniquenessJudge
                    UniquenessJudge.recover();
                    diffFramework.recovery(classPath, className, args, jvmOptions);
                }catch (Exception e){
                    e.printStackTrace();
            ...
            G.reset();  // important!!
            Main.initial(args);
            MutateClass mc = new FlatMutateClass();
            State s = new State();
...
//                MutateClass.setWantReload(false);
    ...
    public static State recoverStateFromBackPath(String classPath, String className, String[] args, String jvmOptions, String backPath) {
        G.reset();  // important!!
        Main.initial(args);
        MutateClass mc = new FlatMutateClass();
        State s = new State();
...
//            System.out.println("Processing " + cp);
            ...
            if(!population.exists())
                continue;
            List<File> fileList = diffFramework.getDifferentialPaths();
            fileList.addAll(Arrays.asList(population.listFiles()));
            for(File f: fileList){
                if (f.getName().endsWith(".class")){
...
//        DjfuzzFramework.dumpSingleMutateClass(s.getTarget(), directory);
    ...
    public static void main(String[] args) throws Exception {
        DjfuzzFrameworkResumableDiffGuide fwk = new DjfuzzFrameworkResumableDiffGuide();

//        Main.useJunit("./junit-4.12.jar", "./hamcrest-core-1.3.jar",
//                "", "com.djfuzz.HelloTest");
//        EnvironmentSetting.generateJunitJvmAndCmd("");
//        fwk.process("com.djfuzz.Hello", 10, args,
//                "./sootOutput/", "", "");
...
//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.mail.MailMessageTest");
//        fwk.process("org.apache.tools.mail.MailMessage", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.tar.TarEntryTest");
//        fwk.process("org.apache.tools.tar.TarEntry", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.tar.TarOutputStreamTest");
//        fwk.process("org.apache.tools.tar.TarOutputStream", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");


//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.ZipEntryTest");
//        fwk.process("org.apache.tools.zip.ZipEntry", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.AsiExtraFieldTest");
//        fwk.process("org.apache.tools.zip.AsiExtraField", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.ExtraFieldUtilsTest");
//        fwk.process("org.apache.tools.zip.ExtraFieldUtils", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.ZipLongTest");
//        fwk.process("org.apache.tools.zip.ZipLong", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.ZipShortTest");
//        fwk.process("org.apache.tools.zip.ZipShort", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");

//        Main.useJunit("../junit-4.12.jar", "../hamcrest-core-1.3.jar",
//                "../tools.jar", "org.apache.tools.zip.ZipOutputStreamTest");
//        fwk.process("org.apache.tools.zip.ZipOutputStream", 10000, args,
//                "./sootOutput/junit-ant/",
//                "", "");
